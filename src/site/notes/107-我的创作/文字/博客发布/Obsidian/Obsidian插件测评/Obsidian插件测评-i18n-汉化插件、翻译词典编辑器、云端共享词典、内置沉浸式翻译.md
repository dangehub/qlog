---
{"uid":20250110223123,"title":"Obsidian插件测评——i18n：汉化插件、翻译词典编辑器、云端共享词典、内置沉浸式翻译","tags":["obsidian插件"],"description":null,"author":"曲淡歌","draft":false,"editable":false,"modified":20250112010131,"dg-publish":true,"dg-path":"Obsidian/Obsidian插件测评/Obsidian插件测评-i18n-汉化插件、翻译词典编辑器、云端共享词典、内置沉浸式翻译.md","permalink":"/Obsidian/Obsidian插件测评/Obsidian插件测评-i18n-汉化插件、翻译词典编辑器、云端共享词典、内置沉浸式翻译/","dgPassFrontmatter":true,"noteIcon":""}
---


# 插件简介

i18n 是一款用于翻译 Obsidian 插件的插件，入门用户可以一键下载其他用户分享的成品词典，进阶用户可以使用内置编辑器提取待译插件中需要翻译的部分并翻译，插件支持百度翻译与 ChatGPT ，也内置了沉浸式翻译用于应急，最后用户可以选择分享自己的词典，插件内置了贡献用户榜单，截止 2025 年 01 月 10 日，已翻译了近 300 款插件。
...
i18n 是 internationalization 的缩写，意为“国际化”。除此处的 i18n，其他均指此插件。
...
也就是说一个插件显示在用户眼前的所有文字信息都被存放在 `main.js` 文件中，最原始的翻译方法就是找到对应文字，再将其替换为翻译后的文字。
...
在上一步我们得到了一个待翻译的词典，这一步我们需要翻译这个词典。也就是说我们要把 第二个 `apple` 翻译为 `苹果`，把一个词典中所有的“值”都翻译完之后，一个成品词典就诞生了，这个时候所有的“值”的集合就可以被称为译文，但是如何在全过程中称呼这个文件呢？用译文去指代一个刚刚被提取的文件似乎不严谨，因此我们就用“词典”来称呼它吧。

为了方便翻译，i18n 也提供了机器翻译接口，包括百度翻译与兼容 OpenAI 格式的大语言模型接口。其中百度翻译 api 每个人都能申请到免费的，而兼容 OpenAI 格式则能让用户使用几乎所有的大语言模型 AI。

#### 注入/替换

当我们完成翻译后，原插件没有发生变化，我们需要根据“键”与“值”的关系，用我们翻译好的内容一一替换原文，这个过程被我称之为“注入”。

#### 云端

虽然我们已经尽可能的简化了翻译流程，但是依然需要用户具备一定的动手能力，因此我们设计了云端词典库，我们借用 Gitee 的 issue 区作为中转区，把用户提交的词典放到中转区，再由志愿者管理员手动检查词典是否有原则性错误（精力、水平有限，只审核明显错误，不能控制翻译质量），审核通过后词典会被存放到 i18n 插件开源库中，新手用户可以在插件中下载对应插件的词典。我们对云端词库的预期就是“一个满足小白用户的、开箱即用的词典库”。
...
3. 如果本地不存在词典，且对应插件云端已有分享词典，你会在插件后面看到一个 `下载` 按钮，如果云端没有词典，则没有下载按钮 ![assets/Pasted image 20250111210321.png](/img/user/107-%E6%88%91%E7%9A%84%E5%88%9B%E4%BD%9C/%E6%96%87%E5%AD%97/%E5%8D%9A%E5%AE%A2%E5%8F%91%E5%B8%83/Obsidian/Obsidian%E6%8F%92%E4%BB%B6%E6%B5%8B%E8%AF%84/assets/Pasted%20image%2020250111210321.png)
	- 如果**本地已有词典**，`下载` 按钮也**不会**出现，需要删除本地词典
4. 点击 `下载` 按钮后，会出现下载面板，点击保存按钮即可 ![assets/Pasted image 20250111210013.png](/img/user/107-%E6%88%91%E7%9A%84%E5%88%9B%E4%BD%9C/%E6%96%87%E5%AD%97/%E5%8D%9A%E5%AE%A2%E5%8F%91%E5%B8%83/Obsidian/Obsidian%E6%8F%92%E4%BB%B6%E6%B5%8B%E8%AF%84/assets/Pasted%20image%2020250111210013.png)
...
*如果某一个插件没有云端词典怎么办？*，为了解决这个问题，i18n 把知名浏览器插件 `沉浸式翻译` 也集成在内。使用方法也很简单：
...
因为 i18n 只是简单的引入了沉浸式翻译的 sdk，因此有很多功能都无法正常使用，包括但不限于：
...
**注意事项**：i18n 是通过正则表达式来匹配插件中的 UI 文本，但这种方法不是万能的。一般插件作者会使用 `.setName("apple")` 的函数 [^2] ，因此我们可以用正则来匹配这个函数，但是这种方法有几个缺陷：
...
5. 等待插件提交，若成功提交，则会打开一个 Gitee issue 网页，志愿管理员会尽快审核词典。（注：issue 中就是被压缩的词典，如果你需要在外部条件下解码，可以使用网页工具 [^3] 。词典有存在问题，可能会被拒绝，因此如果你希望获得相关反馈，建议按照第 2 步配置个人令牌，这样 Gitee 会有提醒。）
